# In-game engine functions, modified to be easier to call directly.
# When the underlying engine function is not mapped in the running build the call is
# ignored (with a one-time warning) and a zeroed result is returned.
...
def GetNodeAbsoluteTransMatrix(node: basic.TkHandle) -> basic.cTkMatrix34:
    """Get the absolute transform matrix of a node."""
